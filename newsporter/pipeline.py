...
def build_prefabs(rows: list[RawRow], cfg: dict) -> list[Prefab]:
    ...
    for row in rows:
        ...
        date_gmt = (
            random_date_for_range(date_cfg, rng) if date_cfg else ""
        )
...
def run_pipeline(
    ...
    results_fh = (
        results_path.open("w", encoding="utf-8") if not loader.dry_run else None
    )
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    source_id = (
                        item.source_id if isinstance(item, Post) else "?"
                    )
                    log.exception("Upload worker error on %s", source_id)
                    record_result(
                        {"source_id": source_id, "error": f"upload: {e}", "ok": False}
                    )
            finally:
                post_q.task_done()
    ...
    try:
        with ThreadPoolExecutor(
        ...
        ) as up_ex, ThreadPoolExecutor(
            max_workers=transform_workers, thread_name_prefix="xform"
        ) as tr_ex:
            ...
            try:
                ...
                # worker may sleep up to attempts × Retry-After-budget
